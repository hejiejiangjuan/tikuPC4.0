<template>
  <div>
    ...
    <el-dialog title="选择测评" width="60%" :visible.sync="dialogTableVisible">
      <div>
...
export default {
  ...
  methods: {
    ...
    moveAppraisalId(row, type) {
      ...
      that.$axios.post("/api/backstage/everyday/move", params).then(
        res => {
          if (res.data.code == 200) {
            that.getList();
            that.$message({
    ...
    open(row, type) {
      ...
      this.$confirm("是否要" + typeName + "" + row.title, "提示", {
        ...
        .then(() => {
          ...
          params.append("type", type);
          that.$axios.post("/api/backstage/everyday/add", params).then(
            res => {
              if (res.data.code == 200) {
                that.getList();
    ...
    getList() {
      ...
      this.$axios.post("/api/backstage/everyday/list", params).then(
        res => {
          if (res.data.code == 200) {
