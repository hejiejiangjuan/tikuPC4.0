...
export default {
  ...
  methods: {
    ...
    moveAppraisalId(row, type) {
      ...
      that.$axios.post("api/backstage/choiceness/move", params).then(
        res => {
          if (res.data.code == 200) {
            that.getList();
            that.$message({
    ...
    open(row, type) {
      ...
      this.$confirm("是否要" + typeName + "" + row.title, "提示", {
        ...
        .then(() => {
          ...
          params.append("type", type);
          that.$axios.post("api/backstage/choiceness/add", params).then(
            res => {
              if (res.data.code == 200) {
                that.getList();
    ...
    getList() {
      ...
      this.$axios.post("api/backstage/choiceness/list", params).then(
        res => {
          if (res.data.code == 200) {
