<template>
  <div>
    ...
    >
      ...
      <el-table-column label="付费阶段">
        <template slot-scope="scope">
          <span v-if="scope.row.payStage=='1'">测前</span>
          <span v-if="scope.row.payStage=='2'">测后</span>
        </template>
      </el-table-column>
      <el-table-column label="题目类型">
        <template slot-scope="scope">
          <span v-if="scope.row.questionType=='1'">正常</span>
          <span v-if="scope.row.questionType=='2'">树形</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="250" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button
            type="success"
            size="small"
            @click="compileList(scope.row,0)"
          >编辑</el-button>
          <el-button
            type="warning"
            v-if="scope.row.status == 1"
            size="small"
            @click="soldOut(scope.row,0)"
          >下架</el-button>
          <!-- <el-button
            type="info"
            v-if="scope.row.status == 1"
          ...
          <el-button
            ...
            size="small"
            @click="soldOut(scope.row,1)"
          >上架</el-button>
          <el-button
            type="danger"
            v-if="scope.row.status == 0"
            size="small"
            @click="soldOut(scope.row,2)"
          >删除</el-button>
          <el-button
            type="primary"
            v-if="scope.row.status == 3"
    ...
    <el-dialog title="选择测评" :visible.sync="dialogTableVisible">
      ...
      width="50">
        ...
        <el-table-column label="选择" prop="rid"  width="100" fixed="right">
          <template slot-scope="scope">
            <el-button  v-if="scope.row.rid == null " type="primary" size="small" @click="open(scope.row,0)">推荐</el-button>
